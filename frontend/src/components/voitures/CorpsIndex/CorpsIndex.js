        ...
        <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-200 bg-[#21283B] rounded-lg">
                ...
                <tbody>
                    {corps.map(corps => (
                        <tr key={corps.id}>
                            <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-white">{corps.type[language]}</td>
                            <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                                <Link to={`/corps-update/${corps.id}`}
                                    className="bg-[#F96C25] hover:bg-[#868E9B] text-white font-bold py-2 px-3 rounded-full">
                                    {t("btnEditer")}
                                </Link>
                                <Bouton
